...
| A member column against a relation's `element` column, and the elements of an `in` list | Same family. A list element of another family is **dropped**, which is exact: CEL answers that one comparison with a definite `false`, and `x OR false` is `x` under either polarity. A list with **no** element the column can hold is refused, because that is a mapping error, not a policy. A `null` element is never dropped: it is not a mismatch |
