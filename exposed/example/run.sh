...
"${ADAPTER_DIR}/gradlew" -p "${ADAPTER_DIR}" publishToMavenLocal --no-daemon --console=plain >&2
...
echo "==> gradlew installDist writeResolvedClasspath" >&2
"${ADAPTER_DIR}/gradlew" -p "${EXAMPLE_DIR}" installDist writeResolvedClasspath --no-daemon --console=plain >&2
