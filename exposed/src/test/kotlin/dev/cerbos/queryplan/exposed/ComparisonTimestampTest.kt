...
class ComparisonTimestampTest {
    ...
    fun `a timestamp literal is compared as an instant`() {
        assertSelects("timestamp/equals/literal-instant", "r1", "r5", "c1")
        assertSelects("timestamp/not-equals/literal-instant", "r2", "r3", "r4", "r6", "r7", "f1")
    }
    ...
    fun `an offset literal names the same instant as its UTC spelling`() =
        ...
        assertSelects("timestamp/equals/literal-instant-non-utc-offset", "r1", "r5", "c1")
    ...
    fun `a column that does not name an absolute instant is refused, not guessed`() {
        ...
        val error = assertThrows<UnmappedAttributeException> { Scalars.ids("cast/timestamp/malformed-string") }
        assertTrue(
    ...
    fun `an OffsetDateTime column is bound at UTC through its own column type`() {
        ...
        val filter = ExposedQueryPlanAdapter.toFilter(
            Scalars.wireFixture("timestamp/equals/literal-instant"),
            Options.of(
                cerbosMapping {
