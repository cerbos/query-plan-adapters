...
class NullOperandScanTest {
    ...
    fun `a null comparison under the omitted convention is refused before anything is built`() {
        val error = assertThrows<UnsupportedPlanShapeException> { Scalars.ids("null-eq-missing", omitted) }
        assertTrue(
    ...
    fun `the over-grant that refusal prevents is real, not hypothetical`() =
        ...
        assertSelects("null-eq-missing", "r2", "r4", "r7")
    ...
    fun `a per-attribute declaration beats the call-level option`() {
        ...
        assertDoesNotThrow { Scalars.ids("null-eq", omitted) }
        assertSelects("null-eq", omitted, "r2", "r4", "r7")
    }
    ...
    fun `the declaration is consulted through an enclosing negation too`() =
        ...
        assertSelects("null-not-eq", omitted, "r1", "r3", "r5", "r6", "f1", "c1")

    @Test
    fun `a plan with no null operand at all is unaffected`() {
        assertSelects("optional-ne", omitted, "r1", "r3", "r5", "f1", "c1")
        assertSelects("cs-eq", omitted, "r1", "c1")
    }
    ...
    fun `the scan matches the OPERAND, not an operator allowlist`() {
        ...
        listOf("vf-null-ne", "null-ne").forEach { action ->
        ...
        assertThrows<UnsupportedPlanShapeException> { Scalars.ids("null-eq-missing", omitted) }
    }
}
