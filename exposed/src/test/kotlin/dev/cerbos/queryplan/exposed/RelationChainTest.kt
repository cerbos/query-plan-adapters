...
class RelationChainTest {
    ...
    fun `a to-one column is read as a correlated scalar subquery with no hop guard`() {
        val sql = render(translate("relation/equals/one-hop-string", WITH_PARENT))
    ...
    fun `an absent parent is excluded under both polarities`() {
        assertEquals(listOf("r2"), ids(translate("relation/bare-attribute/one-hop-boolean", WITH_PARENT)))
        ...
        assertEquals(listOf("r4", "r8"), ids(translate("relation/bare-attribute/negated-one-hop-boolean", WITH_PARENT)))
    }

    @Test
    fun `ordering comparisons through a hop keep the parentless rows out`() {
        assertEquals(listOf("r2"), ids(translate("relation/equals/one-hop-number", WITH_PARENT)))
        assertEquals(listOf("r4"), ids(translate("relation/less-than/one-hop", WITH_PARENT)))
        assertEquals(listOf("r2", "r4"), ids(translate("relation/less-or-equal/one-hop", WITH_PARENT)))
        assertEquals(listOf("r8"), ids(translate("relation/greater-than/one-hop", WITH_PARENT)))
        assertEquals(listOf("r2", "r8"), ids(translate("relation/greater-or-equal/one-hop", WITH_PARENT)))
        // Two comparisons over the same hop, each reading its own subquery.
        assertEquals(listOf("r8"), ids(translate("relation/and/one-hop-range", WITH_PARENT)))
    }
    ...
    fun `a two-level to-one chain nulls out at whichever level is missing`() {
        ...
        assertEquals(listOf("r4"), ids(translate("relation/equals/two-hops-boolean", WITH_PARENT)))
        val sql = render(translate("relation/equals/two-hops-boolean", WITH_PARENT))
    ...
    fun `a missing hop makes only its own branch unknown, not the whole disjunction`() {
        ...
        assertEquals(listOf("r2", "r4", "r5"), ids(translate("relation/or/two-hops-or-collection-exists", WITH_PARENT)))
    }

    @Test
    fun `a column reached through a to-many relation is refused rather than aggregated`() {
        val error = runCatching { translate("string/equals/case-sensitive", MEMBER_OF_TO_MANY) }.exceptionOrNull()
    ...
    fun `an unmapped member of a mapped relation is refused rather than guessed at a column name`() {
        val error = runCatching { translate("relation/equals/one-hop-string", PARENT_WITHOUT_MEMBERS) }.exceptionOrNull()
        assertEquals(UnmappedAttributeException::class.java, error?.javaClass)
