...
class TernaryTranslatorTest {
    ...
    fun `a ternary in boolean position walks each branch as if it were written directly`() =
        assertSelects("conditional/ternary/boolean-branches", "r1", "r4")

    @Test
    fun `a ternary as a comparison operand is substituted back into the comparison`() {
        assertSelects("conditional/ternary/compared-to-literal", "r5", "r7")
        assertSelects("conditional/ternary/expression-condition", "r3", "r6")
        assertSelects("conditional/ternary/nested-in-then-branch", "r5", "r7")
    }

    @Test
    fun `a value-first comparison with a ternary operand keeps source order`() =
        assertSelects("conditional/ternary/value-first", "r1", "r5", "r7", "f1")
    ...
    fun `a negated ternary is the complement only where the condition is determined`() {
        ...
        // clean complement.
        assertSelects("conditional/ternary/compared-to-literal", "r5", "r7")
        assertSelects("conditional/ternary/negated", "r1", "r2", "r3", "r4", "r6", "f1", "c1")
    }
    ...
    fun `an UNKNOWN condition excludes the row under BOTH polarities`() {
        ...
        // three.
        assertSelects("conditional/ternary/missing-attribute-condition", "r5")
        assertSelects("conditional/ternary/negated-missing-attribute-condition", "r1", "r3", "r6", "f1", "c1")
    }

    @Test
    fun `nested ternaries on both sides of one comparison recurse`() {
        assertSelects("conditional/ternary/nested-in-both-branches", "r2", "r5", "r6", "r7", "c1")
        assertSelects("conditional/ternary/on-both-sides", "r5", "r7")
    }
