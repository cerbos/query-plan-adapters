...
class ExposedSurfaceTest {
    ...
    fun `where`() {
        val keys = read {
            SurfaceDocs.selectAll().where { filter("cs-eq").toOp() }.map { it[SurfaceDocs.key] }.sorted()
        }
    ...
    @Test
    fun `count`() {
        assertEquals(3L, read { SurfaceDocs.selectAll().where { filter("cs-eq").toOp() }.count() })
        ...
        assertEquals(2L, read { SurfaceDocs.select(SurfaceDocs.key).where { filter("cs-eq").toOp() }.andWhere { EqOp(SurfaceDocs.region, stringParam("emea")) }.count() })
    }
    ...
    fun `a DAO find takes the same predicate`() {
        val keys = read { SurfaceDoc.find(filter("cs-eq").toOp()).map { it.key }.sorted() }
        assertEquals(listOf("a", "b", "c"), keys)
    }
    ...
    fun `an always-allowed plan filters nothing and an always-denied one filters everything`() {
        assertEquals(QueryPlanFilter.AlwaysAllowed, filter("p-has"))
        assertEquals(QueryPlanFilter.AlwaysDenied, filter("in-empty"))
        assertEquals(5L, read { SurfaceDocs.selectAll().where { filter("p-has").toOp() }.count() })
        assertEquals(0L, read { SurfaceDocs.selectAll().where { filter("in-empty").toOp() }.count() })
        ...
        assertEquals(
            ...
            read {
                SurfaceDocs.selectAll()
                    .where { TriLogic.and(filter("p-has").toOp(), EqOp(SurfaceDocs.region, stringParam("emea"))) }
                    .count()
            },
        )
    }
    ...
    private enum class Store(private val url: (Int) -> String, private val driver: String) {
        ...
        fun fresh(body: (QueryPlanFilter) -> Unit) {
            ...
            transaction(database) {
                ...
                body(ExposedQueryPlanAdapter.toFilter(wireFixture("cs-eq"), Options.of(mapping)))
            }
        }
