...
class ReviewMacroDepthTest {
    ...
    @Test
    @Disabled("REVIEW FINDING 7: the literal-collection fold bypasses maxMacroDepth, so the subquery count a plan can carry is unbounded")
    fun `a macro over a literal collection counts towards the macro depth`() {
        ...
        //
        // CollectionTranslator.translate (CollectionTranslator.kt:47-49) returns into
        // foldKnownValues BEFORE PlanWalker.enterMacro, so neither literal level is counted. At
        // `maxMacroDepth = 1` the plan is accepted and the emitted predicate carries 3 x 3 = 9
        // correlated subqueries; at 11 elements a side it would carry 121, and the bound never
        // trips whatever it is set to.
        //
        // Recommended fix: wrap foldKnownValues in `translation.walker.enterMacro(operator) { … }`
        // as the relation branch does, so a fold costs a level like every other macro.
    ...
    fun `the fold really does multiply the subqueries, which is what the bound exists to stop`() {
        // The measurement behind finding 7, kept passing so the number is on the record: two
        // three-element folds over one relation macro emit nine correlated subqueries at
        // `maxMacroDepth = 1`.
        val op = buildOp(nestedFold(), Options.of(MAPPING).withMaxMacroDepth(1))
