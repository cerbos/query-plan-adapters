...
class MembershipTest {
    ...
    fun `membership in a stored collection is an existence test over the element column`() {
        ...
        assertEquals(listOf("r4", "r5"), ids(translate("null/in/null-literal-in-resource-list")))
    }
    ...
    fun `a null list element becomes IS NULL rather than a never-matching IN`() {
        ...
        val sql = render(translate("null/in/null-literal-in-resource-list"))
    ...
    fun `the negation of collection membership is exact over the empty collection`() {
        ...
        assertEquals(listOf("r1", "r2", "r3", "r6", "r7", "r8"), ids(translate("null/in/negated-null-literal-in-resource-list")))
    }
    ...
    fun `hasIntersection over a stored collection matches any element`() {
        ...
        assertEquals(listOf("r2", "r4", "r5", "r6"), ids(translate("null/has-intersection/literal-list-with-null-element")))
    }
    ...
    fun `a value-first hasIntersection is mirrored rather than read backwards`() {
        ...
        assertEquals(listOf("r2", "r5", "r6"), ids(translate("collection/has-intersection/value-first")))
    }
    ...
    fun `attribute-in-attribute compares the member column against the element column`() {
        ...
        assertEquals(listOf("r2", "r4", "r6"), ids(translate("membership/in/field-in-resource-list")))
    }
    ...
    fun `attribute-in-attribute is definite for a null member under the explicit convention`() {
        ...
        assertEquals(listOf("r1", "r3", "r5", "r7", "r8"), ids(translate("membership/in/negated-field-in-resource-list")))
    }
    ...
    fun `membership over a chain requires its leading hop`() {
        ...
        assertEquals(listOf("r2", "r5"), ids(translate("relation/in/to-one-chain")))
        assertEquals(listOf("r3", "r7"), ids(translate("relation/in/negated-to-one-chain")))
    }

    @Test
    fun `hasIntersection over a chain requires its leading hop`() {
        assertEquals(listOf("r3", "r7"), ids(translate("relation/has-intersection/negated-to-one-chain")))
    }
    ...
    fun `a map projection is strict, so a NULL projected column errors the whole intersection`() {
        ...
        assertEquals(listOf("r2", "r6"), ids(translate("collection/has-intersection/mapped-names-with-metacharacters")))
    }
    ...
    fun `scalar membership folds to the equalities CEL itself evaluates`() {
        ...
        assertEquals(listOf("r1"), ids(translate("membership/in/map-literal-keys")))
    }
    ...
    fun `a collection mapped without an element column is refused rather than guessed`() {
        ...
        val error = runCatching { translate("null/in/null-literal-in-resource-list", noElement) }.exceptionOrNull()
    ...
    fun `attribute-in-attribute over a scalar container is refused`() {
        ...
        val error = runCatching { translate("membership/in/field-in-resource-list", scalarContainer) }.exceptionOrNull()
        assertEquals(UnmappedAttributeException::class.java, error?.javaClass)
        assertEquals(
