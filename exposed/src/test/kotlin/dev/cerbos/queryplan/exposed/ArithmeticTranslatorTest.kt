...
class ArithmeticTranslatorTest {
    ...
    inner class DoubleSpace {
        ...
        fun `add, sub, mult and div are emitted rather than solved`() {
            ...
            assertSelects("arithmetic/add/field-plus-constant", "r2", "r5", "r6", "r7")
            assertSelects("arithmetic/subtract/field-minus-constant", "r1", "r2", "r3", "r4", "r5", "f1", "c1")
            assertSelects("arithmetic/multiply/negative-multiplier", "r1", "r2", "r3", "r5", "r6", "r7", "f1", "c1")
            assertSelects("arithmetic/divide/fractional-quotient", "r2")
            assertSelects("arithmetic/divide/fractional-threshold", "r5", "r6", "r7")
        }

        @Test
        fun `a value-first arithmetic comparison mirrors the operator`() =
            assertSelects("arithmetic/add/value-first", "r2", "r5", "r6", "r7")

        @Test
        fun `arithmetic on both sides compares two SQL expressions`() =
            assertSelects("arithmetic/add/on-both-sides", "r3", "r4")
        ...
        fun `a fractional product is exact IEEE, not exact decimal`() =
            ...
            assertSelects("arithmetic/multiply/inexact-fraction-equals")
        ...
        fun `a fractional add is lowered to SQL rather than inverted in Kotlin`() {
            ...
            assertSelects("arithmetic/add/inexact-fraction-equals")
            assertSelects("arithmetic/add/exact-fraction-equals")
            assertSelects("arithmetic/add/inexact-fraction-not-equals", *Scalars.ALL.toTypedArray())
        }
    }
    ...
    inner class DivisionByZero {
        ...
        fun `a zero divisor yields NaN or a signed infinity, not SQL NULL`() {
            ...
            assertSelects("arithmetic/divide/self-division-greater-than", "r1", "r2", "r4", "r5", "r6", "r7", "f1", "c1")
            assertSelects("arithmetic/divide/self-division-not-equals", "r3")
            assertSelects("arithmetic/divide/negated-self-division-equals", "r3")
        }
        ...
        fun `arithmetic COMPOSED on a zero-capable division keeps the non-finite arm`() {
            ...
            assertSelects("arithmetic/add/self-division-plus-constant-greater-than", "r1", "r2", "r4", "r5", "r6", "r7", "f1", "c1")
            assertSelects("arithmetic/add/self-division-plus-constant-not-equals", "r3")
        }
    }
    ...
    inner class NonFiniteConstants {
        ...
        fun `a NaN or infinity folds against the comparison and is never bound`() {
            ...
            assertSelects("comparison/greater-than/nan-from-ternary", "r1", "r3", "r5", "r7", "f1")
            assertSelects("comparison/less-than/nan-from-ternary-value-first", "r1", "r3", "r5", "r7", "f1")
            assertSelects("comparison/greater-than/infinity-from-ternary", "r1", "r3", "r5", "r7", "f1")
            assertSelects("comparison/less-or-equal/nan-from-ternary-on-both-sides")
        }
    ...
    inner class Refusals {
        ...
        fun `mod is refused, because the cast that makes it satisfiable is itself unlowerable`() {
            val error = assertThrows<UnsupportedPlanShapeException> { Scalars.ids("arithmetic/modulo/negative-dividend") }
        ...
        fun `arithmetic over a non-numeric column is refused rather than cast`() {
            ...
            val error = assertThrows<UnmappedAttributeException> {
                Scalars.ids("arithmetic/add/field-plus-constant", Options.of(mapping))
            }
