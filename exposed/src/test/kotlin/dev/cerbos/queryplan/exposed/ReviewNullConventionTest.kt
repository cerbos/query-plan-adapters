...
class ReviewNullConventionTest {
    ...
    fun `membership of an undeclared attribute must not match a NULL element against a NULL member`() {
        ...
        assertEquals(listOf("d2"), ids("membership/in/field-in-resource-list", UNDECLARED))
    }
    ...
    fun `a negated membership must stay UNKNOWN when the member column is NULL`() {
        ...
        assertEquals(
            listOf("d3"),
            idsOf(ExposedQueryPlanAdapter.toFilter(wireFixture("membership/in/negated-field-in-resource-list"), omitted).toOp()),
        )
    }
    ...
    fun `the same undeclared attribute is NOT explicit-null anywhere else in the walk`() {
        ...
        val declaredSql = render(translate("membership/in/field-in-resource-list", DECLARED))
        val undeclaredSql = render(translate("membership/in/field-in-resource-list", UNDECLARED))
    ...
    fun `under the OMITTED option the undeclared member gets the plain equality it should`() {
        ...
        val omitted = Options.of(UNDECLARED)
            .withNullAttributeRepresentation(NullAttributeRepresentation.OMITTED)
        val op = ExposedQueryPlanAdapter.toFilter(wireFixture("membership/in/field-in-resource-list"), omitted).toOp()
        assertFalse(render(op).contains("\"name\" IS NULL"), render(op))
        assertTrue(render(op).contains("REVIEW_DOCS.OWNER IS NULL"), render(op))
        assertEquals(render(translate("membership/in/field-in-resource-list", UNDECLARED)), render(op))
    ...
    fun `the declared explicit-null member keeps the definite reading the corpus pins`() {
        ...
        assertEquals(listOf("d1", "d2"), ids("membership/in/field-in-resource-list", DECLARED))
    }
