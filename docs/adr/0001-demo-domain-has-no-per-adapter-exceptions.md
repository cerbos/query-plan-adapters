...
it must be expressible by all ten adapters, so there is no equivalent of `actions.json`.
...
The shared domain is necessarily thin — roughly the intersection of ten query languages, one of
