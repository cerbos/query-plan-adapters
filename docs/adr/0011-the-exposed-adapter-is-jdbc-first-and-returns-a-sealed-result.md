...
override and which reject before any lookup. The conformance harness translates every case
...
3. **Compile against the floor, test against the floor and the latest.** The main source set always
   ...
   release, which every store leg executes on) or `floor` (1.0.0); an unknown value fails rather
   than falling back. The floor leg runs every suite, the H2 conformance harness included, so the
   claim is proved against the PDP's recorded `check()` decisions rather than against compilation.
...
- **The floor costs one CI leg.** It replays the whole corpus on 1.0.0, so a rendering difference
  between the floor and the baseline that changes a result fails there.
