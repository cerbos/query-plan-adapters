...
a real store, and compares ids against per-row `check()`. That chain is covered for all eleven
...
| `expected.json`          | The **one shared** expectations file. All eleven examples assert against it. |
...
adapter's filter is shape 5. It lives in the corpus rather than in eleven copies inside eleven examples
...
lives in `run.sh`: eleven languages means eleven packaging stories, and putting all eleven in the shared
...
past Java and Node and would add a web framework to ten examples for reasons unrelated to the
...
stripped, so the expectations can carry their own prose without eleven examples reproducing it.
...
by per-page order.** Several of the eleven stores have no total order to paginate by, and an
...
- **Adding a seed row or attribute** means updating `expected.json` in the same commit, and every
  ...
  second.
- **Adding a usage shape** means implementing it in all eleven examples. There is no per-adapter
...
The domain is thin by construction — roughly the intersection of eleven query languages, one of which
