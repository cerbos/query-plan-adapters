...
The adapter is differentially tested against the pinned Cerbos PDP's `check()` decisions, using the canonical check resources and executable SQLAlchemy queries. The PDP is the semantic oracle.

| Classification | Coverage |
| --- | --- |
| Oracle-tested | Actions whose manifest outcome is `matched` |
| Fail-closed corpus shapes | Manifest outcomes marked `rejected`, including nanosecond thresholds, regex `matches()`, ordered list indexing/`get-field`, ambiguous casts, list-valued expressions in condition position, and operations SQL cannot represent faithfully |
