...
class TestGetQuery:
    ...
    def test_get_is_user(self, contact_table, conn):
        ...
        attr = {
            "request.resource.attr.user": contact_table.user,
        }
    ...
    def test_get_lt_query(self, contact_table, conn):
        ...
        attr = {
            "request.resource.attr.age": contact_table.age,
        }
        query = get_query(plan_resource_resp, contact_table, attr)
        res = conn.execute(query).fetchall()
        assert len(res) == 2
    ...
    def test_get_in_single_query(self, contact_table, conn):
        plan_resources_filter = PlanResourcesFilter.from_dict(
            {
                "kind": PlanResourcesFilterKind.CONDITIONAL,
                "condition": {
    ...
    def test_get_in_multiple_query(self, contact_table, conn):
        plan_resources_filter = PlanResourcesFilter.from_dict(
            {
                "kind": PlanResourcesFilterKind.CONDITIONAL,
                "condition": {
    ...
    def test_not_user(self, contact_table, conn):
        # Generated by a policy with `effect: EFFECT_DENY`
        plan_resources_filter = PlanResourcesFilter.from_dict(
            {
                "kind": PlanResourcesFilterKind.CONDITIONAL,
                "condition": {
