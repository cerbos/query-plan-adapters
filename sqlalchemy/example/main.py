...
def cerbos_host() -> str:
    ...
    any other local PDP listens (a ``cerbos server``, a ``docker run``, another project's), so an
    unset ``CERBOS_HOST`` would not fail: it would quietly plan against whatever policies that PDP
    serves and produce a diff against ``demo/expected.json`` that reads as an adapter bug.
    ``demo/README.md`` requires reaching the PDP at ``$CERBOS_HOST``, "never a hardcoded
