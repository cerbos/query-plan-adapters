...
# The demo-domain example application for the Sequel adapter.
#
# It proves PLUMBING, not semantics: that the published gem installs, that `require "cerbos/
# sequel"` resolves from it, and that the dataset it returns composes with the query methods a
# consumer actually reaches for — `where`, `order`, `offset`/`limit`. The conformance harness
# cannot show any of that, because it loads the adapter from source and only ever runs one flat
# filtered query.
#
...
# stdout carries the JSON document and nothing else, so anything Sequel or a gem decides to
# print has to go elsewhere. Captured before the swap, exactly as spring-data's example does.
...
CERBOS_HOST = ENV.fetch("CERBOS_HOST") {
  raise "CERBOS_HOST is not set — run this example through demo/scripts/run-example.sh sequel"
}
...
ATTRIBUTES = {
  ...
  "request.resource.attr.public" => Cerbos::Sequel.field("is_public")
}.freeze
...
def authorized(plan)
  Cerbos::Sequel.query_plan_to_dataset(plan: plan, model: Document, attributes: ATTRIBUTES)
end

def ids(dataset) = dataset.select_map(:id).sort

# The application's OWN predicate, from the corpus. It is never expressed in policy, which is
# the whole point of usage shape 5.
def application_filter(dataset)
  filter = SEEDS.fetch("applicationFilter")
  dataset.where(archived: filter.fetch("archived"), region: filter.fetch("region"))
end

# --- the five usage shapes ------------------------------------------------------------------

# 1. A plain filtered list: the adapter's dataset IS the query.
def filtered(principal_id, action)
  result = plan(principal_id, action)
  {"kind" => result.kind.to_s, "ids" => ids(authorized(result))}
end

# 4. Pagination. The dataset is ordered and walked a page at a time, and what is reported is
...
def paginated(principal_id, action, page_size)
  result = plan(principal_id, action)
  dataset = authorized(result).order(:id)

  page_sizes = []
  collected = []
  offset = 0
  loop do
    page = dataset.limit(page_size, offset).select_map(:id)
    break if page.empty?
...
shapes = {
  ...
  # 2. An unconditional allow. The adapter returns the whole dataset, and every seed row comes
...
REAL_STDOUT.puts(JSON.pretty_generate({"adapter" => "sequel", "shapes" => shapes}))
