#!/usr/bin/env bash
# The demo-domain example for the Sequel adapter.
#
...
echo "==> gem build cerbos-sequel.gemspec" >&2
GEM_FILE="$(cd "${ADAPTER_DIR}" && gem build cerbos-sequel.gemspec --output "${EXAMPLE_DIR}/cerbos-sequel.gem" >&2 && echo "${EXAMPLE_DIR}/cerbos-sequel.gem")"
...
if tar -xOf "${GEM_FILE}" data.tar.gz | tar -tzf - | grep -q '^example/'; then
  echo "cerbos-sequel.gem contains example/ — the example must not ship inside the" \
...
UNPACKED="${EXAMPLE_DIR}/vendor/cerbos-sequel"
...
mv "${EXAMPLE_DIR}"/vendor/cerbos-sequel-*.gemspec \
  "${UNPACKED}/cerbos-sequel.gemspec"
