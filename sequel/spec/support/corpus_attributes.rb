...
module CorpusAttributes
  def self.field(path, **kwargs) = Cerbos::Sequel.field(path, **kwargs)

  def self.omitted(path) = field(path, null_representation: :omitted)

  def self.association(*args, **kwargs) = Cerbos::Sequel.association(*args, **kwargs)
  ...
  ATTRIBUTES = {
    ...
    # each hop becomes a correlated scalar subquery, and a missing parent gives NULL, which denies like
    ...
    "request.resource.attr.tags" => association(
      :tags, fields: {"id" => field("tag_id"), "name" => field("name")}
    ),
    # tags[].name as a list. A NULL name stays in the list as a null element.
    "request.resource.attr.tagNames" => association(:tags, member_field: "name"),

    # An association has no order, so the translator refuses indexing these lists at `index`.
    # Mapped anyway so the refusal names that reason and not a missing mapping.
    "request.resource.attr.aNumberList" => association(:number_list_elements, member_field: "value"),
    "request.resource.attr.aBoolList" => association(:bool_list_elements, member_field: "value"),

    "request.resource.attr.categories" => association(:categories, fields: {
      "name" => field("name"),
      "subCategories" => association(:sub_categories, fields: {
        "name" => field("name"),
        "labels" => association(:labels, fields: {"name" => field("name")})
      })
    }),
    ...
    #
    # Nested on purpose. A flat `many_to_many` cannot tell a missing parent from a parent
    # with no children, so `all`, `!exists` and counts would return the category-less rows the
    # PDP denies (#309).
    "request.resource.attr.mainCategory" => association(:categories, fields: {
      "subCategories" => association(:sub_categories, fields: {"name" => field("name")}),
      "subNames" => association(:sub_categories, member_field: "name")
    })
  }.freeze
  ...
  UNDECLARED = ATTRIBUTES.transform_values { |mapping|
    mapping.is_a?(Cerbos::Sequel::AttributeMapping::Field) ? field(mapping.path) : mapping
  }.freeze
end
