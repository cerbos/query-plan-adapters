...
RSpec.describe "conformance" do
  ...
  # An `unsupported` entry passes on any Cerbos::Sequel::Error, and an attribute the map
    ...
    describe "PDP #{tag}" do
      ...
      ConformanceCorpus.goldens(tag).each do |golden|
        it "#{golden.fetch("tier")}: #{golden.fetch("id")}" do
          ...
          when "unsupported" then expect { ids(plan) }.to raise_error(Cerbos::Sequel::Error)
