...
module Cerbos
  module Sequel
    ...
    #
    # Sequel writes <tt>ESCAPE '\'</tt> after every +LIKE+ that it renders, on every dialect
    # that it supports, so the escape character here is that backslash. The translator unit
    # test asserts over the whole corpus that each +LIKE+ carries the clause, so a Sequel
    # release that stopped writing it would fail there and not in production.
    class StringMatcher
      ESCAPE_CHAR = "\\"
      ...
      def match(receiver, needle, prefix:, suffix:)
        ...
        # :LIKE and not :ILIKE. CEL compares strings with attention to the case of the letters.
        # Sequel writes `LIKE BINARY` on MySQL for this operator, so the collation of a column
        # cannot make it case-insensitive there.
        SqlSupport::BooleanExpression.new(:LIKE, receiver, pattern)
      end
      ...
      def column_pattern(needle, prefix:, suffix:)
        pattern = METACHARACTERS.reduce(needle) do |escaped, metacharacter|
          SqlSupport.function(:REPLACE, [escaped, metacharacter, ESCAPE_CHAR + metacharacter])
        end

        pattern = dialect.concat("%", pattern) if prefix
        pattern = dialect.concat(pattern, "%") if suffix
        pattern
      end
    end
  end
