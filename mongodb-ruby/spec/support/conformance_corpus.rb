...
module ConformanceCorpus
  ...
  SEEDS_FILE = JSON.parse(File.read(File.join(DIR, "seeds.json"), encoding: "UTF-8")).freeze
  ACTIONS_FILE = JSON.parse(File.read(File.join(DIR, "actions.json"), encoding: "UTF-8")).freeze
  DERIVED_FILE = JSON.parse(File.read(File.join(DIR, "derived-fields.json"), encoding: "UTF-8")).freeze
  ...
  SEEDS.each_with_index do |seed, index|
    ...
    # The two homogeneous scalar lists. The driver stores each element with its own BSON type,
    # and the positional actions read that type, so an element of a third type would reach the
    # document and check() as a shape no action here was classified against. A null element is
    # a value in both lists and is stored as one.
    {"aNumberList" => [Numeric], "aBoolList" => [TrueClass, FalseClass]}.each do |key, types|
      ...
      raise "seeds.json seeds[#{index}].#{key} carries #{stray.inspect}, which is neither " \
            "the list's element type nor null"
    end
  end
  ...
  def wire_fixture(action)
    substitute_planned_at(JSON.parse(File.read(File.join(WIRE_FIXTURES_DIR, "#{action}.json"), encoding: "UTF-8")))
  end
