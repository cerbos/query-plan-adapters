...
function withControlPlaneFixture({
  ...
  const rootDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), "drizzle-control-plane-"),
  );
  try {
    fs.mkdirSync(path.join(rootDirectory, "conformance"));
    fs.mkdirSync(path.join(rootDirectory, "drizzle"));
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(rootDirectory, "drizzle", "adapterctl.json"),
      JSON.stringify({ schemaVersion: 1, adapter: "drizzle", outcomes }),
    );
    run(rootDirectory);
  } finally {
  ...
  withControlPlaneFixture({
    ...
    run: (rootDirectory) => {
      expect(() =>
        ...
        "drizzle/adapterctl.json has an unassessed outcome for new-action",
      );
    },
  });
});
  ...
  withControlPlaneFixture({
    ...
    run: (rootDirectory) => {
      expect(() =>
        ...
        "drizzle/adapterctl.json.outcomes must account for every catalog action exactly once",
      );
    },
  });
});
