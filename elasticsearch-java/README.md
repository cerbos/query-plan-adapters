...
strings stored as `keyword` values does not provide reliable instant ordering. If source strings
can be malformed, set `ignore_malformed: true` on that date field: Elasticsearch then keeps the
original value in `_source` without indexing a date. Send that original string to Cerbos. Both a
positive timestamp comparison and its negation exclude the unindexed value, matching CEL's
conversion error; without this mapping option Elasticsearch rejects the entire document. The
corpus proves both polarities with an invalid date string. Ordinary `date`
...
The adapter is differentially tested against Cerbos PDP 0.54.0 `check()` decisions using 27 hostile seed documents and real Elasticsearch queries. The Spring Data adapter defines the reference semantics for this compatibility snapshot.
...
| Fail-closed | 159 reference actions plus ordered list indexing/`get-field`, `int()`/`double()` casts and `filter()`/`map()` used as a condition or a conjunct (168 actions total) |
...
that raise them**. 169 of the corpus's 290 shapes are refused here — the 168 fail-closed actions
above plus `null-eq-missing` — so it matters whether that happens at one catch-all or at many. It
is 29 sites, with 72 actions reaching the computed-operand refusal:

| Rejection site | Actions |
| --- | --- |
| computed leaf operand | 72 |
| field-to-field | 22 |
| explicit null | 8 |
