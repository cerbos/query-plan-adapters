...
| Fail-closed | 175 reference actions plus ordered list indexing/`get-field`, `int()`/`double()` casts and `filter()`/`map()` used as a condition or a conjunct (184 actions total) |
...
refusals over the sites in the walk that raise them**. 185 of the corpus's 327 shapes are refused
here — the 184 fail-closed actions plus `null-eq-missing` — across 29 sites, with 83 reaching the
