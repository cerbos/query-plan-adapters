...
function withControlPlaneFixture({
  ...
  const rootDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), "mongoose-control-plane-"),
  );
  try {
    fs.mkdirSync(path.join(rootDirectory, "conformance"));
    fs.mkdirSync(path.join(rootDirectory, "mongoose"));
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(rootDirectory, "mongoose", "adapterctl.json"),
      JSON.stringify({ schemaVersion: 1, adapter: "mongoose", outcomes }),
    );
    run(rootDirectory);
  } finally {
  ...
  withControlPlaneFixture({
    ...
    run: (rootDirectory) => {
      expect(() =>
        ...
        "mongoose/adapterctl.json has an unassessed outcome for new-action",
      );
    },
  });
});
  ...
  withControlPlaneFixture({
    ...
    run: (rootDirectory) => {
      expect(() =>
        ...
        "mongoose/adapterctl.json.outcomes must account for every catalog action exactly once",
      );
    },
  });
});
